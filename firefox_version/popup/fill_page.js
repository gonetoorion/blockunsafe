window.onload = function(){
    ...
    function get_url(tabs){
        ...
        if(config_url.indexOf(orignal_f_key)==-1){
            let lan = browser.i18n.getUILanguage();
            lan = lan.toLowerCase();
            let lan_prefix = "";
            if(lan=="zh-cn"){
                lan_prefix = "z";
            ...
            document.getElementById(add_kyw_id).setAttribute("href", config_url.replace("config", config_lan)+orignal_f_key+"&lhn="+last_hostname+"&cltp=3");
    ...
    function start_up(){
        tab_info = browser.tabs.query({currentWindow: true, active: true});
        tab_info.then(get_url, deal_error);
    }
    start_up();
